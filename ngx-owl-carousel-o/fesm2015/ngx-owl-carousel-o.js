...
ResizeService.decorators = [{ type: Injectable }];
ResizeService.ctorParameters = () => [{ type: EventManager }];
...
OwlLogger.decorators = [{ type: Injectable }];
OwlLogger.ctorParameters = () => [{ type: ErrorHandler }];
...
class CarouselService {
            ...
            if (checkedOptions.hasOwnProperty(key)) {
                ...
                if (mockedTypes[key] === 'number') {
                    if (this._isNumeric(checkedOptions[key])) {
                        ...
                        checkedOptions[key] =
                            key === 'items' ? this._validateItems(checkedOptions[key], checkedOptions.skip_validateItems) : checkedOptions[key];
                    } else {
    ...
    pointer(event) {
        ...
        event =
            event.touches && event.touches.length ? event.touches[0] : event.changedTouches && event.changedTouches.length ? event.changedTouches[0] : event;
        if (event.pageX) {
...
CarouselService.decorators = [{ type: Injectable }];
CarouselService.ctorParameters = () => [{ type: OwlLogger }];
...
class NavigationService {
     ...
     */
    spyDataStreams() {
        const initializedCarousel$ = this.carouselService.getInitializedState().pipe(
            tap(state => {
                this.initialize();
        ...
        const changedSettings$ = this.carouselService.getChangedState().pipe(
        ...
        const refreshedCarousel$ = this.carouselService.getRefreshedState().pipe(
            tap(() => {
                this._updateNavPages();
                this.draw();
                this.update();
...
NavigationService.decorators = [{ type: Injectable }];
NavigationService.ctorParameters = () => [{ type: CarouselService }];
...
BrowserWindowRef.decorators = [{ type: Injectable }];
...
BrowserDocumentRef.decorators = [{ type: Injectable }];
...
class AutoplayService {
    ...
    spyDataStreams() {
        ...
        const resized$ = this.carouselService.getResizedState().pipe(
            tap(() => {
                if (this.carouselService.settings.autoplay) {
                    this.play();
                } else {
...
AutoplayService.decorators = [{ type: Injectable }];
...
LazyLoadService.decorators = [{ type: Injectable }];
LazyLoadService.ctorParameters = () => [{ type: CarouselService }];
...
AutoHeightService.decorators = [{ type: Injectable }];
AutoHeightService.ctorParameters = () => [{ type: CarouselService }];
...
HashService.decorators = [{ type: Injectable }];
...
CarouselSlideDirective.decorators = [{ type: Directive, args: [{ selector: 'ng-template[carouselSlide]' }] }];
CarouselSlideDirective.ctorParameters = () => [{ type: TemplateRef }];
...
class CarouselComponent {
    ...
    spyDataStreams() {
        ...
        this._changedCarousel$ = this.carouselService.getChangeState().pipe(
            switchMap(value => {
                const changedPosition = of(value).pipe(
        ...
        this._draggingCarousel$ = this.carouselService.getDragState().pipe(
            ...
            switchMap(() => this.carouselService.getDraggedState().pipe(map(() => !!this.carouselService.is('animating')))),
            switchMap(anim => {
                if (anim) {
...
StageComponent.ctorParameters = () => [{ type: NgZone }, { type: ElementRef }, { type: Renderer2 }, { type: CarouselService }, { type: AnimateService }];
...
OwlRouterLinkDirective.decorators = [{ type: Directive, args: [{ selector: ':not(a)[owlRouterLink]' }] }];
...
OwlRouterLinkWithHrefDirective.decorators = [{ type: Directive, args: [{ selector: 'a[owlRouterLink]' }] }];
OwlRouterLinkWithHrefDirective.ctorParameters = () => [{ type: Router }, { type: ActivatedRoute }, { type: LocationStrategy }];
...
export {
    ...
    SlidesOutputData,
    NavigationService as ɵa,
    CarouselService as ɵb,
    OwlLogger as ɵc,
    AutoplayService as ɵd,
    WINDOW as ɵe,
    WindowRef as ɵf,
    BrowserWindowRef as ɵg,
    windowFactory as ɵh,
    browserWindowProvider as ɵi,
    windowProvider as ɵj,
    WINDOW_PROVIDERS as ɵk,
    DOCUMENT as ɵl,
    DocumentRef as ɵm,
    BrowserDocumentRef as ɵn,
    documentFactory as ɵo,
    browserDocumentProvider as ɵp,
    documentProvider as ɵq,
    DOCUMENT_PROVIDERS as ɵr,
    LazyLoadService as ɵs,
    AnimateService as ɵt,
    AutoHeightService as ɵu,
    HashService as ɵv,
    ResizeService as ɵw,
    StageComponent as ɵx,
};
