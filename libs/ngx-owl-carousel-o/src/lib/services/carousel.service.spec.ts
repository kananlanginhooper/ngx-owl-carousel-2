...
describe('CarouselService in context of TestComponent', () => {
    ...
    beforeEach(
        waitForAsync(() => {
            TestBed.configureTestingModule({
                imports: [NoopAnimationsModule, RouterTestingModule.withRoutes([{ path: '', component: TestComponent }])],
                declarations: [CarouselComponent, TestComponent, CarouselSlideDirective, StageComponent],
                providers: [ResizeService, WINDOW_PROVIDERS, CarouselService, NavigationService, DOCUMENT_PROVIDERS, OwlLogger],
            });
        }),
    );
