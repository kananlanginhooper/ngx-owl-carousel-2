...
export class HashService implements OnDestroy {
    ...
    constructor(private carouselService: CarouselService, @Optional() private route: ActivatedRoute, @Optional() private router: Router) {
        this.spyDataStreams();
        if (!this.route) {
            this.route = {
     ...
     */
    spyDataStreams() {
        const initializedCarousel$: Observable<string> = this.carouselService.getInitializedState().pipe(tap(() => this.listenToRoute()));
